...
ncsa_endpoint_old = (
    "http://mbobak.ncsa.illinois.edu:9999/blazegraph/namespace/nabu/sparql"
)
ncsa_endpoint_ = (
    "https://mbobak.ncsa.illinois.edu:9999/blazegraph/namespace/nabu/sparql"
)
ncsa_endpoint = "https://mbobak.ncsa.illinois.edu:9999/bigdata/namespace/ld/sparql"
dev_https_endpoint = (
    "https://graph.geocodes-dev.earthcube.org/blazegraph/namespace/https/sparql"
)
gc1_endpoint = (
    "https://graph.geocodes-1.earthcube.org/blazegraph/namespace/earthcube/sparql"
)
...
summary_endpoint = dflt_endpoint.replace(
    "earthcube", "summary"
)  # can't always just switch
# from 'sources' gSheet: can use for repo:file_leaf naming/printing
base_url2repo = {
    "https://raw.githubusercontent.com/earthcube/GeoCODES-Metadata/main/metadata/Dataset/json": "geocodes_demo_datasets",
...
def test_sitemaps(sitemaps=named_sitemaps):
    ...
    sc = sitemaps_count(
        sitemaps
    )  # but needs to handle timeouts before running that full list
...
def replace_base(
    path, mydict=base_url2repo, sep=":"
):  # for context like: repo:filename
...
def blabel_l(fn):
    ...
    ext = file_ext(fn)
    cs = (
        f"java -Xmx4155M -jar {jar}/blabel.jar  LabelRDFGraph -l -i {fn} -o {fb}_l{ext}"
    )
    os_system(cs)
...
def df_diff(
    df1, df2
):  # doesn't work as well as I would like in all situations, work on/fix/finish
    import pandas as pd
...
def check_urn_ld_cache(
    ...
    test_jsonld = (
        f"{test_base}{test_bucket}/summoned/{test_set}/"  # {urn}.jsonld added later
    )
...
def check_urn_diffs_(
    ...
    if testing_bucket != "citesting":  # to change test_bucket ...
        ...
        jsonld_checks = list(
            map(check_urn_jsonld, dfu)
        )  # could do after, only if a problem, or just check them all now
...
def install_fuseki(
    url="https://dlcdn.apache.org/jena/binaries/apache-jena-fuseki-4.5.0.tar.gz",
):
    return install_url(url)
...
def get_relateddatafilename_txt(
    url="https://raw.githubusercontent.com/MBcode/ec/master/NoteBook/sparql_relateddatafilename.txt",
):
    return get_ec_txt(
        url
    )  # need var to be {?q} so dont have to write extra logic below


def get_webservice_txt(
    url="https://raw.githubusercontent.com/earthcube/facetsearch/master/client/src/sparql_blaze/sparql_gettools_webservice.txt",
):
    return get_ec_txt(url)


def get_download_txt(
    url="https://raw.githubusercontent.com/earthcube/facetsearch/master/client/src/sparql_blaze/sparql_gettools_download.txt",
):
    return get_ec_txt(url)


def get_notebook_txt(
    url="https://raw.githubusercontent.com/MBcode/ec/master/NoteBook/sparql_gettools_notebook.txt",
):
    return get_ec_txt(url)
...
def get_query_txt(
    url="https://raw.githubusercontent.com/MBcode/ec/master/NoteBook/sparql-query.txt",
):
    return get_ec_txt(url)
...
def get_oss_(
    minio_endpoint_url="https://oss.geocodes-dev.earthcube.org/",
):  # old version
    import s3fs

    oss = s3fs.S3FileSystem(
        anon=True, key="", secret="", client_kwargs={"endpoint_url": minio_endpoint_url}
    )
    return oss
...
def wget_oss_repo(repo=None, path="gleaner/milled", bucket=gc1_minio):
    ...
    if not repo:
        ...
        repo = path_leaf(cwd)
        print(
            f"using, repo:{repo}=path_leaf({cwd})"
        )  # as 2nq.py will use cwd for repo, if it runs .rdf files
...
def set_collectionName(collectionName):
    ...
    collection = (
        f"<{subj}/collection/{collectionName}>"  # get_collectionID(collectionName)
    )
...
def get_rdf2nt(urn):
    ...
    if not is_str(fn2):
        print(
            f"get_rdf2nt,warning,fn2:{fn2}, make sure to run the Parameters cell to get the urn"
        )
        fn2 = ""
    fn2 = fn2 + ".nt"
    global f_nt
...
def wget_rdf_(urn, viz=None):
    ...
    # if(urn!=None and urn.startswith('urn:')):
        ...
        url, urlj = urn2urls(
            urn
        )  # so can reuse this, also getting sys change/fining missing in minio
        urlroot = path_leaf(url)  # file w/o path
        cs = f"wget -a log {url}"
        os_system(cs)
        cs = f"wget -a log {urlj}"
        ...
        url = urn.replace("/", "http://geocodes.ddns.net/ld/", 1).replace(
            ".jsonld", ".nt", 1
        )
        urlroot = path_leaf(url)  # file w/o path
        # url += ".nt"
...
def getDatasetURLs(IDs, dfS=None):
    ...
    ds_urls = (
        list(map(urn2accessURL, IDs))
        if d1p
        else list(map(lambda row: dfRow2urls(dfS, row), IDs))
    )  # or put in another row number to get url
    ...
    ds_url = list(
        map(lambda urls: urls[0], ds_urls)
    )  # default to 1st of the urls ;need to check in 2nd/sparql_nb w/o collection
...
def sparql_f2(
    fq, fn, r=None
):  # jena needs2be installed for this, so not in NB yet;can emulate though
    "files: qry,data"
    if (
        r
...
def viz(
    fn=".all.nt",
):  # might call this rdf_viz once we get some other type of viz going
    if has_ext(fn):
        ext = file_ext(fn)
...
def read_file(fnp, ext=None):  # download url and ext/filetype
    ...
    fn = fnp.rstrip(
        "/"
    )  # only on right side, for trailing slash, not start of full pasted path
    fn1 = path_leaf(fn)  # just the file, not it's path
    fext = file_ext(fn1)  # &just it's .ext
    # url = fn
    if ext != None:
    ...
    if ext == None and len(ft) < 1:
        ...
        ft == ".tsv"
        or re.search("tsv", ext, re.IGNORECASE)
        or re.search("tab-sep", ext, re.IGNORECASE)
    ):
        try:
        ...
        try:
            ...
            df = str(sys.exc_info()[0])
            pass
    elif ft == ".txt" or re.search(
        "text", ext, re.IGNORECASE
    ):  # want to be able to header=None here
        try:
            df = pd.read_csv(fn, sep="\n", comment="#")
        except:
            df = str(sys.exc_info()[0])
...
# ---- sources:
def get_sources_csv(
    url="https://raw.githubusercontent.com/MBcode/ec/master/crawl/sources.csv",
):
    return get_ec_txt(url)
...
def get_summary(
    g="",
):  # g not used but could make a version that gets it for only 1 graph
...
def get_graphs_cache(
    endpoint="http://ideational.ddns.net:9999/bigdata/namespace/nabu/sparql",
    dumpfile=None,
):
...
def show_related(
    df, row
):  # after dfCombineFeaturesSimilary is run on your df 'sparql results'
...
def prov2mappings(urls):  # use urls from p above
    ...
    for url in urls:
        ...
        value2 = value.split(":")[
            -1
        ]  # so can also lookup form UUID w/o urn:... before it
...
def bucket_files(url):  # need to get past: <MaxKeys>1000</MaxKeys>
    ...
    if c:
        ...
        dates = map(
            lambda kd: kd.get("LastModified"), c
        )  # or when get prov look at file dates before the parse?
...
def endpoint_description(
    url="https://graph.geocodes-dev.earthcube.org/blazegraph/namespace/citesting2/sparql",
):
...
LD_cache_files = None
LD_cache_dates = (
    None  # Might use, or as we query the file, also want to get files metadata
)
LD_cache_types = None
...
def get_oss_files_(
    ...
    bucket="gleaner",
    path=None,
    minio_endpoint_url="https://oss.geodex.org/",
    full_path=True,
):  # usually do not use version ending in _
    if not bucket:
        global testing_bucket
...
def cmp_expected_results(
    df=None,
    df2="https://raw.githubusercontent.com/MBcode/ec/master/test/expected_results.csv",
):
...
def csv_dropoff(
    sitemap_url="https://raw.githubusercontent.com/MBcode/ec/master/test/sitemap.xml",  # start expecte bad cases
    bucket_url=None,
    ...
    sm = sitemap_list(
        sitemap_url
    )  # can now use sitemap2urn to get sitemap into same ID space
    ...
    sm_ru = list(
        map(replace_base, sm)
    )  # acts as key for each dict ;need v of replace_base w/base_url2repo, but for url
    ...
    csv_out = df.set_axis(
        ["repo:file_name", "summoned", "milled", "graph"], axis=1, inplace=False
    )
    if cmp or dbg:
...
def set_prov2site_mappings():  # make sure it is run, to get mappings
    ...
    pu = get_bucket_files(
        f"prov/{repo_name}"
    )  # only the ones for the repo_name being run
    ...
    if pu:
        ...
        for (
            k,
            v,
        ) in urn2site_urls.items():  # =list(map(urn2uuid,urn2site_urls));needs is_urn
            k2 = urn2uuid(k)
            if k2:
...
def drop1mapping(
    sm, sitemap2urn, su
):  # want to but not used yet; still in crawl_dropoff
    "break out all the code in crawl_dropoff that deals w/sitemap to summoned w/mapping"
    sml = len(sm)
    # print(f'will get:{sml} urn2urn w/:{sitemap2urn}') #dbg
    # smu=list(map(lambda s: sitemap2urn[s], sm))
    smu = list(
        map(lambda s: sitemap2urn.get(s), sm)
    )  # used prov mapping to gen test sitemap
    if dbg:
        print(
            f"URN/UUIDs for sitemaps:{smu}"
        )  # need to get same sitemap, &use map for cmp:
    # smu2=list(map(lambda s: s.split(':')[-1],smu))
    smu2 = list(map(lambda s: s if not s else s.split(":")[-1], smu))  # =urn_tails(smu)
    if dbg:
        print(
            f"URN/UUIDs for sitemaps:{smu2}"
        )  # need to get same sitemap, &use map for cmp:
    lose_s2s = list_diff_dropoff(smu2, su)
    lsl = len(lose_s2s)  # should= sml- ml from above
    print(
        f"lose_s2s:{lose_s2s}"
    )  # will need to map this back to the sitemap url ;if it was in prov
...
def sitemap_dropoff(sitemap_url=None, bucket_url=None):  # bucket to get prov's version
    ...
    if sitemap_url:
        ...
        sm = []
    if bucket_url:
        sitemap_l = prov2sitemap(
            bucket_url
        )  # this gives the list of them, which most fncs expect the sitemap_url
    else:
...
def spot_crawl_dropoff(sitemap, bucket_url, endpoint):
    ...
    if not lose_m2u:
        ...
        m_check = list(
            map(check_urn_rdf, lose_m2u)
        )  # can't do this if no milled,  can try the transformation though
...
def tsc(
    sitemap=None,
    bucket_url=None,
    endpoint="https://graph.geocodes-dev.earthcube.org/blazegraph/namespace/citesting/sparql",
):
    ...
    print(f"=s3_endpoint:{bucket_url}")
    print(f"=graph_endpoint:{endpoint}")
    sitemap_l, sm, dropoff1 = sitemap_dropoff(
        sitemap, bucket_url
    )  # will still get sitemap_l if no sitemap url given
...
def tsc2__(
    sitemap2="http://geocodes.ddns.net/ec/test/sitemap.xml",
    bucket_url2=None,
    endpoint2=None,
):
...
def tsc2_(
    bucket_url=None,
    endpoint2="https://graph.geocodes-dev.earthcube.org/blazegraph/namespace/citesting2/sparql",
):
...
def tsc3(
    sitemap="https://raw.githubusercontent.com/MBcode/ec/master/test/sitemap.xml",
    bucket_url=None,
    endpoint="https://graph.geocodes-dev.earthcube.org/blazegraph/namespace/citesting2/sparql",
):
    "as is being used in spot_test/report on geocodes-dev now"  # still need to deal w/milled dissapearing in a few places
...
def tsc4_(
    sitemap="https://earthcube.github.io/GeoCODES-Metadata/metadata/Dataset/allgood/sitemap.xml",
    bucket_url=None,
    endpoint="https://graph.geocodes-dev.earthcube.org/blazegraph/namespace/citesting2/sparql",
):
    "as is being used in spot_test/report on geocodes-dev now"  # still need to deal w/milled dissapearing in a few places
